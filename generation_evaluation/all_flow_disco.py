...
def handle(idx, iter_count, folder_path, error_index, error_name):
...
if __name__ == "__main__":
    ...
    folder_path = "./result/explanation/{}/exp_disco/iter=".format(group) + str(iter_count) + "/sample=" + str(sample_num) + "/"
    ...
    if mode == "explain":
        ...
        [pool.apply_async(handle, args=(idx, iter_count, folder_path, error_index, error_name,), callback=update, error_callback=print_error) for idx in target_idx_list]
        pool.close()
        pool.join()
        ...
        while error_index:
            ...
            error_name[:] = []
            pool = multiprocessing.Pool(processes=10)
            [pool.apply_async(handle, args=(idx, iter_count, folder_path, error_index, error_name,), callback=update, error_callback=print_error) for idx in temp_error_index]
            pool.close()
            pool.join()
            print("Error Name: ", error_name)
            print("Error Index: ", error_index)
    ...
    if mode == "evaluate":
        ...
        evaluation_folder_path = "./result/evaluation/{}/k=".format(group) + str(evaluation_k) + "/exp_disco/" + "iter=" + str(iter_count) + "/sample=" + str(sample_num) + "/"
