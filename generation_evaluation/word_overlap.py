...
def preprocess_text(text):
    # Tokenization, lowercasing, removing stop words, lemmatization, and stemming
    tokens = [token.lemma_ for token in nlp(text) if token.is_alpha and token.text.lower() not in stop_words]
    stemmed_tokens = [stemmer.stem(token) for token in tokens]
    return set(stemmed_tokens)

def calculate_word_overlap(explanation, user_inquiry_history):
    explanation_words = preprocess_text(explanation)
    user_inquiry_words = preprocess_text(user_inquiry_history)
    common_words = explanation_words.intersection(user_inquiry_words)
    return len(common_words) / len(user_inquiry_words) if len(user_inquiry_words) > 0 else 0
        ...
        if os.path.exists(explanation_path):
            with open(explanation_path, "r") as file:
                ...
                if "personalized_explanation" in explanation:
                    ...
                    explanation = json.dumps(explanation)
            overlap_rate = calculate_word_overlap(explanation, user_inquiry_history)
            total_overlap_rate += overlap_rate
            count += 1

    if count > 0:
        average_overlap_rate = total_overlap_rate / count
        print(f'Average word overlap rate: {average_overlap_rate:.4f}')
    else:
